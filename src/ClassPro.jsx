      ...
      {isMember ? (
        <div className="group-composer">
          ...
          <div className="group-composer-row">
            ...
            <textarea rows={1} placeholder="Écrire au groupe…" value={text} onChange={(e) => setText(e.target.value)} />
...
function ParametresOverlay({ profile, onClose, onLogout, appSettings, updateAppSettings, exportMyData, deleteAccount, accounts, toggleBlock, onOpenLegal }) {
  ...
  if (step === "verify") {
    return (
      <div className="verify-box">
        ...
        <p className="muted small verify-demo-note">Mode démo (pas de backend réel) — voici le code qu'un vrai SMS/e-mail contiendrait : <b>{genCode}</b></p>
        <div className="form-row">
    ...
    <div className="phone">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .phone { font-family:'Inter',sans-serif; color:var(--ink); width:100%; max-width:430px; margin:0 auto; height:100svh; max-height:900px; background:var(--bg); position:relative; overflow:hidden; border-radius:28px; box-shadow:0 20px 60px rgba(21,19,43,0.28); display:flex; flex-direction:column; }
        ...
        .modal-backdrop { position:absolute; inset:0; background:rgba(21,19,43,0.45); display:flex; align-items:flex-end; z-index:20; }
        .modal-sheet { background:var(--surface); width:100%; border-radius:24px 24px 0 0; padding:18px 18px 24px; max-height:85%; overflow-y:auto; }
        ...
        .chat-overlay { position:absolute; inset:0; background:var(--bg); z-index:30; display:flex; flex-direction:column; }
      ...
      {!profile ? (
        ...
        <>
          ...
          {overlay === "parametres" && <ParametresOverlay profile={profile} onClose={() => setOverlay(null)} onLogout={logout} appSettings={appSettings} updateAppSettings={(c) => setAppSettings((s) => ({ ...s, ...c }))} exportMyData={exportMyData} deleteAccount={deleteAccount} accounts={accounts} toggleBlock={toggleBlock} onOpenLegal={() => setOverlay("mentions")} />}
