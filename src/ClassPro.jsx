...
// mobiles limitées, mais seulement quand ça vaut le coup : les fichiers déjà légers sont
// envoyés tels quels, et les gros fichiers sont compressés en qualité quasi sans perte.
// Retombe sur le fichier original si la compression échoue ou si le format ne s'y prête pas
// (ex. GIF animé).
function compressImage(file, { maxDim = 1920, quality = 0.9, skipBelow = 400 * 1024 } = {}) {
  return new Promise((resolve) => {
    if (!file.type.startsWith("image/") || file.type === "image/gif" || file.size <= skipBelow) {
    ...
    img.onload = () => {
      ...
      // Si la "compression" finit par peser plus lourd que l'original (arrive sur les
      // petits PNG very compressibles), autant garder l'original.
      if (compressedSize >= file.size) {
        readFileAsDataUrl(file).then((original) => resolve({ dataUrl: original, originalSize: file.size, compressedSize: file.size }));
        return;
      }
...
function ProfileForm({ initial, onDone, submitLabel = "Continuer", editMode = false }) {
  ...
  const handleVerifDoc = async (e) => {
    ...
    if (file.type.startsWith("image/")) {
      const { dataUrl } = await compressImage(file, { maxDim: 1800, quality: 0.85 });
